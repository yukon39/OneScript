...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("Тест_ДолженПроверитьОбъединениеПутейГдеВторойПараметрЯвляетсяКорнем");
	ВсеТесты.Добавить("Тест_ДолженПроверитьОбъединениеПутейГдеТретийПараметрЯвляетсяКорнем");
...
Процедура Тест_ДолженПроверитьОбъединениеПутейГдеВторойПараметрЯвляетсяКорнем() Экспорт
	Разделитель = ПолучитьРазделительПути();
	
	Путь1 = "КаталогА" + Разделитель;
	Путь2 = "КаталогБ";
	
	юТест.ПроверитьРавенство(Путь1 + Путь2, ОбъединитьПути(Путь1, Разделитель + Путь2));
	
	Путь1 = "КаталогА";
	Путь2 = "КаталогБ";
	
	юТест.ПроверитьРавенство(Путь1 + Разделитель + Путь2, ОбъединитьПути(Путь1, Разделитель + Путь2));
	
КонецПроцедуры

Процедура Тест_ДолженПроверитьОбъединениеПутейГдеТретийПараметрЯвляетсяКорнем() Экспорт
	Разделитель = ПолучитьРазделительПути();
	
	Путь1 = "КаталогА";
	Путь2 = "КаталогБ" + Разделитель;
	Путь3 = "КаталогВ";
	
	юТест.ПроверитьРавенство(Путь1 +  Разделитель + Путь2 + Путь3, ОбъединитьПути(Путь1, Путь2, Разделитель + Путь3), "Путь1 +  Разделитель + Путь2 + Путь3, ОбъединитьПути(Путь1, Путь2, Разделитель + Путь3)");
	
	Путь1 = "КаталогА";
	Путь2 = "КаталогБ";
	Путь3 = "КаталогВ";
	
	юТест.ПроверитьРавенство(Путь1 + Разделитель + Путь2 + Разделитель + Путь3, ОбъединитьПути(Путь1, Путь2, Разделитель + Путь3), "Путь1 + Разделитель + Путь2 + Разделитель + Путь3, ОбъединитьПути(Путь1, Путь2, Разделитель + Путь3)");
